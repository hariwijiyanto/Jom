...
def main():
    ...
    parser.add_argument('--reduction-step', type=lambda x: int(x, 0), default=2**70,
                        help='Langkah pengurangan untuk DP (dalam hex atau decimal, misal 0x100000)')
    parser.add_argument('--num-dp', type=int, default=1024, help='Jumlah DP points')
    parser.add_argument('--bloom-fp-rate', type=float, default=0.001, help='False positive rate Bloom Filter')
    parser.add_argument('--max-start-scalars', type=int, default=162259276829213363391578010288128,
                       help='Maksimal jumlah start scalar yang akan dicoba')
    ...
    # Inisialisasi DP table dengan BigInt reduction factor
    bloom_size = optimize_bloom_filter_size(args.num_dp, args.bloom_fp_rate)
    dp_table_size = args.num_dp

    # Ubah dtype DP entry untuk menyimpan BigInt
    dp_table_dtype = np.dtype([
        ('fp', np.uint64),
        ('reduction_factor', np.uint32, 8)  # 8 uint32 = 256-bit BigInt
    ])
    ...
    # Konversi reduction-step ke BigInt array
    reduction_step_np = int_to_bigint_np(args.reduction_step)
    
    print(f"[*] Membuat {args.num_dp} DP points di GPU...")
    print(f"[*] Reduction step: {hex(args.reduction_step)}")
    print(f"[*] Reduction step BigInt: {reduction_step_np}")
    
    block_size_dp = 256
    grid_size_dp = (dp_table_size + block_size_dp - 1) // block_size_dp

    # Panggil kernel dengan BigInt reduction step
    generate_dp_kernel(
        d_dp_table, d_bloom_filter,
        np.uint32(dp_table_size),
        np.uint64(bloom_size),
        cuda.In(reduction_step_np),  # Kirim sebagai BigInt
        block=(block_size_dp, 1, 1),
        grid=(grid_size_dp, 1)
    )
    cuda.Context.synchronize()

    # Sort DP table berdasarkan fingerprint
    dp_table_host = np.zeros(dp_table_size, dtype=dp_table_dtype)
    cuda.memcpy_dtoh(dp_table_host, d_dp_table)
    
    # Sort by fingerprint (uint64)
    sorted_indices = np.argsort(dp_table_host['fp'])
    dp_table_host_sorted = dp_table_host[sorted_indices]
    cuda.memcpy_htod(d_dp_table, dp_table_host_sorted)
