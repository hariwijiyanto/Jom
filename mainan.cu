...
struct DPEntry {
    uint64_t fp;
    BigInt reduction_factor; // Ubah ke BigInt
};
...
__device__ bool find_dp_factor_binary(
    const DPEntry* dp_table,
    uint32_t dp_table_size,
    uint64_t fp,
    BigInt* reduction_factor_out
) {
    int left = 0;
    ...
    while (left <= right) {
        ...
        if (mid_fp == fp) {
            copy_bigint(reduction_factor_out, &dp_table[mid].reduction_factor);
            return true;
        } else if (mid_fp < fp) {
            left = mid + 1;
        } else {
...
__global__ void find_pubkey_kernel_sequential(
    ...
    if (check_bloom_filter(d_bloom_filter, bloom_size, fp_val)) {
        ...
        if (find_dp_factor_binary(d_dp_table, dp_table_size, fp_val, &reduction_factor_bi)) {
...
__global__ void generate_dp_table_kernel(
    ...
    uint32_t dp_table_size,
    uint64_t bloom_size,
    const BigInt* reduction_step_bigint  // Parameter BigInt
) {
    ...
    // 1. HITUNG REDUCTION FACTOR: (idx+1) * reduction_step
    BigInt idx_bigint;
    init_bigint(&idx_bigint, idx + 1);
    
    BigInt reduction_factor_bi;
    scalar_mul_mod_n(&reduction_factor_bi, &idx_bigint, reduction_step_bigint);
    ...
    // 4. HITUNG DP POINT: Target - reduction_factor*G
    ECPointJac dp_jac;
    ...
    // 7. SIMPAN KE DP TABLE
    d_dp_table[idx].fp = fp;
    copy_bigint(&d_dp_table[idx].reduction_factor, &reduction_factor_bi);
